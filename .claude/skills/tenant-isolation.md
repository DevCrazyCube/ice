...
The authenticated organization ID must be attached to every request early in the middleware chain:
...
Every repository function enforces `organization_id`:
...
When a resource exists but belongs to a different organization, return `404 NOT_FOUND`, not `403 FORBIDDEN`. This prevents leaking existence information.

## Audit

Any function that accesses business data without `organization_id` is a security defect. Review all repository functions before shipping any data phase.

## Anti-Patterns

- Querying by only `id` without `organization_id`
